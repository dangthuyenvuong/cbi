...
export class File {
    ...
    static replace(fileName, object = {}) {
        let data = fs.readFileSync(fileName, 'utf-8');
        for (let i in object) {
            let regexp = new RegExp(`(${i})`, 'g')
            data = data.replace(regexp, `${object[i]}`)
        }
        fs.writeFileSync(fileName, data)
    }
    ...
    static readJsonFile(target) {
        let rawdata = fs.readFileSync(target);
        return JSON.parse(rawdata)
    }

    static writeJsonFile(target, data){
        return fs.writeFileSync(target, JSON.stringify(data, null, 2))
    }

    static updateJsonFile(target, object = {}) {
        let data = this.readJsonFile(target)
        for (let i in object) {
            if (typeof data[i] === 'object' && typeof object[i] === 'object') {
                data = {
                    ...data,
                    [i]: {
                        ...data[i],
        ...
        return this.writeJsonFile(target, data)
    }
}
