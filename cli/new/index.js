...
import fs from 'fs'
import { url, logger, __awaiter, File } from "../../lib/utils/index.js";
...
const handle = async (inputs, options) => {
    try {
        let { name } = inputs
        ...
        let questions = [
            {
                type: 'list',
                message: chalk.yellowBright('Chọn framework bạn muốn sử dụng?'),
                name: 'framework',
                choices: ['Reactjs', 'Vuejs', 'Angularjs'],
            },
            {
                type: 'list',
                message: chalk.yellowBright('Chọn loại dự án bạn muốn?'),
                name: 'system_type',
                choices: [SYSTEM_TYPE.REACT, SYSTEM_TYPE.NEXT],
                when: (answer) => answer.framework === 'Reactjs'
            },
            {
                type: 'checkbox',
                message: chalk.yellowBright('Chọn package được cài đặt sẵn?'),
                name: 'module',
                choices: ['tailwind'],
                when: (answer) => answer.framework === 'Reactjs',
            },
            {
                type: 'checkbox',
                message: chalk.yellowBright('Chọn thư viện được cài đặt sẵn?'),
                name: 'module',
                choices: ['authentication', 'cart', 'user'],
                when: (answer) => answer.framework === 'Reactjs',
            },
            {
                type: 'list',
                message: chalk.yellowBright('Chọn system UI bạn muốn sử dụng?'),
                name: 'system_design',
...
export const setupCreateCli = (program) => {
    program
        .command('new [name]')
